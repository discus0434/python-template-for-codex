...
- 何もやるにもまずはプランを作成する。要件の不明瞭な部分はユーザーに質問し、全てが明確になるまで仕様策定を続ける。詳細は[仕様追加・変更のライフサイクル](#仕様追加・変更のライフサイクル)を参照。
...
- 実装が一通り終わったら、docs更新に入る前に **plan-reviewer** を使って別エージェントにレビューさせる。
- プランと現コード・テストの乖離がないことを確認し、乖離があれば修正する。
...
- プランの作成後や、実装の完了後には適宜 plan-reviewer や plan-implementation-reviewer でセルフレビューを行い、重要な問題を洗い出して修正する。
- AGENTは、プランにある内容を実装し終えたら plan-implementation-reviewer でセルフレビューし、重要な乖離を解消してから、ユーザー承認→docs反映へ進む。
- つまり、AGENTの仕様追加・変更のライフサイクルは以下のようになる。
    1. ユーザーからの要望を受け取り、不明確な部分があればユーザーに質問し、全てが明確になるまで仕様策定を続ける。
    2. 仕様策定が完了したら、`plans/` 下にプランを記述する。
    3. plan-reviewer でセルフレビューし、重要な不足/矛盾/無駄/不明瞭さを修正する。なお、ユーザーの希望があれば、この工程は省略してもよい。
    4. プランにある内容を実装する。[ルール](#ルール)に従ってTDDで実装を行い、テストを通す。
    5. plan-implementation-reviewer でセルフレビューし、プランとコード/テストの乖離を修正する。
    6. 実装が完了したら、ユーザーから承認を得る。承認を得たら、`plans/` 下からプランを `docs/archive/` に移動し、`docs/` 下のドキュメントを更新し、変更概要を `docs/CHANGELOG.md` に追記する。
