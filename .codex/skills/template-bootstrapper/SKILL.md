...
## Quickstart
1. クリーンコピーを用意（または既存リポをclone）
2. パッケージ名を決める: 例) プロジェクト名 `awesome-api` → パッケージ名 `awesome_api`
3. モジュールリネーム: `mv src/python_template_for_codex src/<package>`
4. プロジェクト名・パッケージ名を書き換える（手動）
   - 出現箇所を確認: `rg "python_template_for_codex"` / `rg "Python Template for Codex"`（閲覧のみ）
   - エディタで `<package>` / `<Project Title>` に置換（自動置換コマンドは使わない）
5. `pyproject.toml` を更新
   - `[project] name = "<project-name>"`
   - `dependencies`/`optional-dependencies`があれば必要に応じて調整
6. READMEを更新（タイトル、概要、セットアップ手順）
